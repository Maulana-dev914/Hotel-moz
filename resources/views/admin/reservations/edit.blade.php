...
@section('title', 'Editar Reserva - Hotel Moz')

@section('content')
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h2 class="mb-0" style="font-weight: 700; color: var(--text-primary);">Editar Reserva</h2>
        <p class="text-muted mb-0" style="font-size: 0.9rem;">Atualizar dados da reserva</p>
    </div>
    ...
    <div class="card-body-modern">
        <form action="{{ route('admin.reservations.update', $reservation) }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <select class="form-select" id="guest_id" name="guest_id" required>
                    @foreach($guests as $guest)
                        <option value="{{ $guest->id }}" {{ old('guest_id', $reservation->guest_id) == $guest->id ? 'selected' : '' }}>{{ $guest->name }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="mb-4">
                ...
                @php
                    $currentRoomIds = $reservation->rooms->count() > 0 
                        ? $reservation->rooms->pluck('id')->toArray() 
                                ...
                                <div class="card-body">
                                    <div class="form-check">
                                        <input class="form-check-input room-checkbox" type="checkbox" 
                                            ...
                                            {{ (old('room_ids') && in_array($room->id, old('room_ids'))) || in_array($room->id, $currentRoomIds) ? 'checked' : '' }}>
            ...
            <div class="mb-3">
                ...
                <select class="form-select" id="status" name="status" required>
                    <option value="pendente" {{ old('status', $reservation->status) === 'pendente' ? 'selected' : '' }}>Pendente</option>
                    <option value="confirmada" {{ old('status', $reservation->status) === 'confirmada' ? 'selected' : '' }}>Confirmada</option>
            ...
            <button type="submit" class="btn btn-primary" id="submit-btn">Atualizar</button>
        </form>
    </div>
</div>
